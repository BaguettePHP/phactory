...
class Blueprint
{
    ...
    public function __construct($name, $defaults, $associations = [], Phactory $phactory = null)
    {
    ...
    public function create($overrides = [], $associated = [])
    {
        ...
        foreach ($associated as $name => $row) {
            ...
            if ($association instanceof Association\ManyToMany) {
                ...
                $many_to_many[$name] = [$row, $association];
            }
        }
    ...
    protected function _associateManyToMany($row, $many_to_many)
    {
        ...
        foreach ($many_to_many as $name => $arr) {
            ...
            foreach ($to_rows as $to_row) {
                ...
                $r = $stmt->execute([':from_id' => $row->getId(), ':to_id' => $to_row->getId()]);
